...
function Workspace({ onLogout, userName, userId, publicId }: { onLogout: () => void; userName: string; userId?: number; publicId?: string }) {
  ...
  const playVoiceRoomTone = (kind: "join" | "leave") => { const AudioContextClass = window.AudioContext ?? (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (!AudioContextClass) return; const context = new AudioContextClass(); const oscillator = context.createOscillator(); const gain = context.createGain(); oscillator.type = "sine"; oscillator.frequency.setValueAtTime(kind === "join" ? 660 : 420, context.currentTime); oscillator.frequency.exponentialRampToValueAtTime(kind === "join" ? 880 : 280, context.currentTime + 0.14); gain.gain.setValueAtTime(0.045, context.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, context.currentTime + 0.18); oscillator.connect(gain); gain.connect(context.destination); oscillator.start(); oscillator.stop(context.currentTime + 0.18); void context.close(); };
  const previousVoiceParticipants = useRef<Set<number> | null>(null);
  useEffect(() => { if (!activeVoiceId || !voiceParticipantsQuery.data) { previousVoiceParticipants.current = null; return; } const current = new Set(voiceParticipantsQuery.data.map((entry) => entry.member.userId)); const previous = previousVoiceParticipants.current; if (previous) { if (Array.from(current).some((id) => !previous.has(id))) playVoiceRoomTone("join"); if (Array.from(previous).some((id) => !current.has(id))) playVoiceRoomTone("leave"); } previousVoiceParticipants.current = current; }, [activeVoiceId, voiceParticipantsQuery.data]);
