...
function Workspace({ onLogout, userName, userId, publicId }: { onLogout: () => void; userName: string; userId?: number; publicId?: string }) {
  ...
  useEffect(() => { webrtc.setScreenShareEndedListener(() => { setScreenSharing(false); toast.info("O compartilhamento de tela foi encerrado."); }); return () => webrtc.setScreenShareEndedListener(undefined); }, [webrtc]);
  ...
  const toggleScreenShare = async () => { if (screenSharing) { webrtc.stopScreenShare(); setScreenSharing(false); toast.info("Compartilhamento de tela encerrado"); return; } try { await webrtc.shareScreen(); setScreenSharing(true); toast.success("Compartilhamento de tela iniciado"); } catch (error) { const name = error instanceof DOMException ? error.name : ""; if (name === "AbortError") { toast.info("Compartilhamento de tela cancelado pelo usuário."); } else if (name === "NotAllowedError") { toast.error(window.top !== window.self ? "O compartilhamento de tela precisa ser aberto na aba principal do Concord, não dentro de um quadro incorporado." : "O navegador não abriu o seletor de tela. Recarregue o Concord em uma aba HTTPS, clique novamente no botão e escolha uma tela, janela ou aba na janela nativa do navegador."); } else if (name === "NotReadableError") { toast.error("A fonte de tela não pôde ser lida. Feche outro compartilhamento ativo e tente novamente."); } else { toast.error(error instanceof Error ? error.message : "Não foi possível abrir o seletor de tela."); } } };
