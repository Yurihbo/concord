...
function Workspace({ onLogout, userName, userId, publicId }: { onLogout: () => void; userName: string; userId?: number; publicId?: string }) {
  ...
  const joinVoice = async (voiceId: number, voiceName: string) => { try { if (activeVoiceId === voiceId) { voiceStream?.getTracks().forEach((track) => track.stop()); voiceActivityMutation.mutate({ channelId: voiceId, isSpeaking: false }); leaveVoiceMutation.mutate({ channelId: voiceId }); webrtc.dispose(); setVoiceStream(null); setActiveVoiceId(null); setMuted(false); setVoiceLevel(0); lastPublishedSpeaking.current = null; toast.success(`Você saiu de ${voiceName}`); return; } if (activeVoiceId) { voiceActivityMutation.mutate({ channelId: activeVoiceId, isSpeaking: false }); }
    lastPublishedSpeaking.current = null;
    voiceStream?.getTracks().forEach((track) => track.stop()); const stream = await webrtc.captureMicrophone(); webrtc.startMicrophoneMeter(); await joinVoiceMutation.mutateAsync({ channelId: voiceId }); setVoiceStream(stream); setActiveVoiceId(voiceId); setActiveChannel(voiceName); setView("home"); setMobilePanel("closed"); toast.success(`Você entrou em ${voiceName}`); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível entrar na sala de voz."); } };
