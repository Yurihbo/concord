...
export default function FirebaseWorkspace() {
  ...
  useEffect(() => {
    if (!community || !voiceRoomId) { setVoiceMessages([]); return; }
    return subscribeToChannelMessages(community.id, voiceRoomId, setVoiceMessages, (error) => setNotice(error.message));
  }, [community, voiceRoomId]);
  ...
  const sendVoiceChat = async () => {
    if (!community || !voiceRoomId || voiceSending || (!voiceBody.trim() && !voiceFile)) return;
    if (voiceFile && voiceFile.size > 25 * 1024 * 1024) { setNotice("O arquivo excede o limite de 25 MB."); return; }
    setVoiceSending(true);
    try {
      const attachment = voiceFile ? await uploadVoiceChatFile(community.id, voiceRoomId, currentUser.uid, voiceFile) : undefined;
      await sendVoiceChatMessage(community.id, voiceRoomId, currentUser.uid, voiceBody.trim() || `Arquivo enviado: ${voiceFile?.name ?? "anexo"}`, attachment);
      setVoiceBody("");
      setVoiceFile(null);
  ...
  const leaveVoiceRoom = async () => {
    ...
    setVoiceStream(null); setVoiceRoomId(null); setMuted(false); setLocalSpeaking(false); setScreenSharing(false); setScreenPreviewStream(null); setScreenViewerIds(new Set()); setRemoteStreams({}); setRemoteScreenStreams({}); setPeerQualities({}); setScreenVolumes({}); previousScreenBroadcasters.current = new Set();
