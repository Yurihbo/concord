...
import { Bell, ChevronDown, Compass, GripVertical, Hash, Headphones, Home as HomeIcon, Link2, LogOut, MessageCircle, Mic, MoreHorizontal, PhoneOff, Plus, Radio, Search, Send, Settings, Signal, Sparkles, Trash2, Upload, UserPlus, Users, Video, Volume2, WandSparkles, X } from "lucide-react";
...
export default function FirebaseWorkspace() {
  ...
  const leaveVoiceRoom = async () => {
    ...
    setVoiceStream(null); setVoiceRoomId(null); setVoiceSessionId(null); setMuted(false); setLocalSpeaking(false); setScreenSharing(false); setScreenPreviewStream(null); setScreenViewerIds(new Set()); setRemoteStreams({}); setRemoteScreenStreams({}); setPeerQualities({}); setScreenVolumes({}); setPeerVolumes({}); setSelectedPeerVolumeId(null); setVoiceAudioBlocked(false); setVoiceAudioUnlockVersion(0); previousScreenBroadcasters.current = new Set();
