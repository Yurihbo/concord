...
import { Bell, ChevronDown, Compass, GripVertical, Hash, Headphones, Home as HomeIcon, Link2, LogOut, MessageCircle, Mic, MicOff, MoreHorizontal, PhoneOff, Plus, Radio, Search, Send, Settings, Signal, Sparkles, Trash2, Upload, UserPlus, Users, Video, Volume2, WandSparkles, X } from "lucide-react";
...
async function captureDirectStream(media: "audio" | "screen", inputDeviceId?: string, noiseSuppression = true): Promise<MediaStream> {
  ...
  if (media === "audio") return navigator.mediaDevices.getUserMedia({ audio });
...
function readVoiceInviteFromLocation(): { communityId: string; roomId: string } | null {
  if (typeof window === "undefined") return null;
  const params = new URLSearchParams(window.location.search);
  const communityId = params.get("voiceCommunity")?.trim();
  const roomId = params.get("voiceRoom")?.trim();
  return communityId && roomId ? { communityId, roomId } : null;
}

function buildVoiceInviteUrl(communityId: string, roomId: string): string {
  const url = new URL(window.location.href);
  url.search = new URLSearchParams({ voiceCommunity: communityId, voiceRoom: roomId }).toString();
  url.hash = "";
  return url.toString();
}
...
function ProfilePanel({ profile, email, onSave, onUpload }: { profile: FirebaseProfile | null; email: string; onSave: (displayName: string, avatarUrl: string) => void; onUpload: (file: File) => Promise<string> }) {
  ...
  useEffect(() => { setPreviewFailed(false); }, [avatarUrl]);
  ...
  return <section className="workspace-panel profile-workspace-panel"><div className="workspace-panel-heading"><div><span className="firebase-kicker">CONCORD / PERFIL</span><h1>Seu perfil</h1><p>Personalize o nome e a imagem que aparecem para suas conexões.</p></div><Settings size={27} /></div><div className="profile-edit-card"><div className="profile-avatar-preview"><div className="profile-avatar-image">{avatarUrl && !previewFailed ? <img src={avatarUrl} alt="Prévia do avatar" onError={() => setPreviewFailed(true)} /> : <span>{initials(displayName || "Conta")}</span>}</div><div><strong>Imagem do perfil</strong><span>Escolha um avatar pronto ou envie sua própria imagem.</span></div></div><div className="profile-avatar-section"><span className="profile-field-label">Avatares prontos</span><div className="avatar-preset-grid">{presetAvatars.map((avatar) => <button type="button" className={avatarUrl === avatar.url ? "avatar-preset selected" : "avatar-preset"} key={avatar.url} onClick={() => setAvatarUrl(avatar.url)} aria-label={`Usar avatar ${avatar.name}`}><img src={avatar.url} alt={avatar.name} /></button>)}</div><label className="avatar-upload-button"><Upload size={15} /> Enviar imagem própria<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={chooseFile} /></label></div><label>Nome de exibição<Input value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={40} /></label><label>E-mail<Input value={email} readOnly /></label><div className="profile-code"><span>Seu código público</span><strong>{profile?.publicId ?? "CON-00000000"}</strong></div><Button className="primary-cta" onClick={() => onSave(displayName.trim(), avatarUrl)} disabled={!displayName.trim()}>Salvar perfil</Button></div></section>;
}

function SettingsPanel({ audioDevices, selectedInput, selectedOutput, noiseSuppression, micLevel, onInputChange, onOutputChange, onNoiseChange, onRefreshDevices }: { audioDevices: MediaDeviceInfo[]; selectedInput: string; selectedOutput: string; noiseSuppression: boolean; micLevel: number; onInputChange: (deviceId: string) => void; onOutputChange: (deviceId: string) => void; onNoiseChange: (enabled: boolean) => void; onRefreshDevices: () => void }) {
  const inputs = audioDevices.filter((device) => device.kind === "audioinput");
  const outputs = audioDevices.filter((device) => device.kind === "audiooutput");
  return <section className="workspace-panel settings-workspace-panel"><div className="workspace-panel-heading"><div><span className="firebase-kicker">CONCORD / CONFIGURAÇÕES</span><h1>Configurações</h1><p>Ajuste sua voz e a forma como você usa o Concord.</p></div><Settings size={27} /></div><div className="settings-grid"><div className="settings-card voice-settings-card"><div className="voice-settings-card-heading"><Mic size={19} /><span><strong>Áudio da chamada</strong><small>O Concord reconhece os dispositivos usados atualmente e permite trocar quando quiser.</small></span><button type="button" className="settings-refresh-button" onClick={onRefreshDevices} aria-label="Atualizar dispositivos de áudio" title="Atualizar dispositivos"><Radio size={15} /></button></div><div className="voice-settings-fields"><label>Entrada de voz<select value={selectedInput} onChange={(event) => onInputChange(event.target.value)}><option value="">Padrão do sistema</option>{selectedInput && !inputs.some((device) => device.deviceId === selectedInput) && <option value={selectedInput}>Microfone atual</option>}{inputs.map((device) => <option key={device.deviceId} value={device.deviceId}>{device.label || "Microfone disponível"}</option>)}</select></label><label>Saída de voz<select value={selectedOutput} onChange={(event) => onOutputChange(event.target.value)}><option value="">Padrão do sistema</option>{selectedOutput && !outputs.some((device) => device.deviceId === selectedOutput) && <option value={selectedOutput}>Saída atual</option>}{outputs.map((device) => <option key={device.deviceId} value={device.deviceId}>{device.label || "Saída disponível"}</option>)}</select></label></div><label className="voice-toggle-row"><span><strong>Supressão de ruído</strong><small>Reduz ruídos contínuos do ambiente durante as chamadas.</small></span><input type="checkbox" checked={noiseSuppression} onChange={(event) => void onNoiseChange(event.target.checked)} /></label><div className="mic-level-card"><div><strong>Nível do microfone</strong><span>{Math.round(micLevel * 100)}%</span></div><div className="mic-level-track" role="progressbar" aria-label="Nível do microfone" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(micLevel * 100)}><i style={{ width: `${Math.round(micLevel * 100)}%` }} /></div><small>Entre em uma call para testar o sinal em tempo real.</small></div></div><div className="settings-card static"><UserPlus size={19} /><span><strong>Privacidade</strong><small>Suas solicitações e convites usam somente os dados necessários para entrar nas comunidades.</small></span></div><div className="settings-card pwa-settings-card"><span><strong>Aplicativo Concord</strong><small>Instale o Concord na tela inicial para abrir suas comunidades como um aplicativo.</small></span><PwaInstallButton /></div></div></section>;
}
...
function DraggableScreenPanel({ stream, title, label, volume, muted = true, outputDeviceId, onVolumeChange, onClose }: { stream: MediaStream; title: string; label: string; volume?: number; muted?: boolean; outputDeviceId?: string; onVolumeChange?: (value: number) => void; onClose?: () => void }) {
...
function RemoteAudioMixer({ streams, volumes, unlockVersion, outputDeviceId, onBlocked }: { streams: Record<string, MediaStream>; volumes: Record<string, number>; unlockVersion: number; outputDeviceId?: string; onBlocked: () => void }) {
  const audioRef = useRef<HTMLAudioElement>(null);
  const onBlockedRef = useRef(onBlocked);
  onBlockedRef.current = onBlocked;
  useEffect(() => {
    const audio = audioRef.current;
    const entries = Object.entries(streams).filter(([, stream]) => stream.getAudioTracks().length);
    ...
    if (AudioContextCtor) {
      ...
      for (const [peerId, stream] of entries) {
        ...
        gain.gain.value = Math.max(0, Math.min(2, volumes[peerId] ?? 1));
    ...
    } else combinedStream = new MediaStream(entries.flatMap(([, stream]) => stream.getAudioTracks()));
  ...
  }, [streams, volumes, unlockVersion, outputDeviceId]);
...
export default function FirebaseWorkspace() {
  ...
  const [messageProfiles, setMessageProfiles] = useState<Record<string, FirebaseProfile>>({});
  ...
  const [voiceProfiles, setVoiceProfiles] = useState<Record<string, FirebaseProfile>>({});
  const voiceParticipantIds = useMemo(() => Array.from(new Set(members.map((member) => member.uid))).sort(), [members]);
  ...
  const [mobileDrawer, setMobileDrawer] = useState<"channels" | "members" | null>(null);
  const [membersOpen, setMembersOpen] = useState(true);
  const [notice, setNotice] = useState("");
  const [loading, setLoading] = useState(false);
  const [audioDevices, setAudioDevices] = useState<MediaDeviceInfo[]>([]);
  const [selectedInput, setSelectedInput] = useState(() => typeof window === "undefined" ? "" : window.localStorage.getItem("concord.voice.input") ?? "");
  const [selectedOutput, setSelectedOutput] = useState(() => typeof window === "undefined" ? "" : window.localStorage.getItem("concord.voice.output") ?? "");
  const [noiseSuppression, setNoiseSuppression] = useState(() => typeof window === "undefined" ? true : window.localStorage.getItem("concord.voice.noiseSuppression") !== "false");
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setMobileDrawer(null); };
  ...
  const [peerVolumes, setPeerVolumes] = useState<Record<string, number>>(() => {
    ...
    try { return JSON.parse(window.localStorage.getItem("concord.voice.peerVolumes") ?? "{}") as Record<string, number>; } catch { return {}; }
  });
  const [selectedPeerVolumeId, setSelectedPeerVolumeId] = useState<string | null>(null);
  const [mutedAllParticipants, setMutedAllParticipants] = useState(false);

  useEffect(() => {
    try { window.localStorage.setItem("concord.voice.peerVolumes", JSON.stringify(peerVolumes)); } catch { /* armazenamento local indisponível */ }
  }, [peerVolumes]);
  ...
  useEffect(() => {
    const refreshAudioDevices = async () => {
      try { setAudioDevices(await voiceService.listAudioDevices()); } catch { setAudioDevices([]); }
    };
  ...
  useEffect(() => {
    if (!voiceStream) { setMicLevel(0); return; }
    const timer = window.setInterval(() => setMicLevel(voiceService.getMicrophoneLevel()), 120);
    return () => window.clearInterval(timer);
  }, [voiceService, voiceStream]);
  ...
  useEffect(() => {
    ...
    if (!audio || !selectedOutput || !("setSinkId" in audio)) return;
    void (audio as HTMLAudioElement & { setSinkId: (deviceId: string) => Promise<void> }).setSinkId(selectedOutput).catch(() => undefined);
  }, [selectedOutput, directRemoteStream]);
  ...
  useEffect(() => {
    const authorIds = Array.from(new Set(messages.map((message) => message.authorId).filter(Boolean)));
    if (!authorIds.length) { setMessageProfiles({}); return; }
    let cancelled = false;
    void getProfiles(authorIds).then((profiles) => {
      if (cancelled) return;
      setMessageProfiles(Object.fromEntries(profiles.map((author) => [author.uid, author])));
    }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [messages]);
  ...
  useEffect(() => {
    if (!community || !channel || channel.kind !== "voice") { setMembers([]); setVoiceProfiles({}); voiceRosterIdsRef.current = null; return; }
  ...
  useEffect(() => {
    if (!voiceParticipantIds.length) { setVoiceProfiles({}); return; }
    return subscribeToProfiles(voiceParticipantIds, (profiles) => {
      setVoiceProfiles(Object.fromEntries(profiles.map((profile) => [profile.uid, profile])));
    }, (error) => setNotice(error.message));
  }, [voiceParticipantIds.join(",")]);
  ...
  const deleteMessage = async (message: FirebaseMessage) => {
    if (!community || message.authorId !== currentUser.uid) return;
    if (!window.confirm("Apagar esta mensagem? Ela será removida do armazenamento do grupo.")) return;
    try {
      await deleteChannelMessage(community.id, message.id, currentUser.uid);
      setNotice("Mensagem apagada do grupo.");
    } catch (error) {
  ...
  const startDirectCall = async (media: "audio" | "screen") => {
    ...
    try {
      const local = await captureDirectStream(media, selectedInput || undefined, noiseSuppression);
  ...
  const acceptDirectCall = async () => {
    ...
    stopDirectCallRingtone();
    try {
      const local = await captureDirectStream("audio", selectedInput || undefined, noiseSuppression);
  ...
  const applyVoicePreferences = async (inputDeviceId = selectedInput, nextNoiseSuppression = noiseSuppression) => {
    if (voiceRoomId && voiceStream) {
      const nextStream = await voiceService.replaceMicrophone(inputDeviceId || undefined, nextNoiseSuppression);
      await meshRef.current?.replaceMicrophone(nextStream);
      setVoiceStream(nextStream);
      await voiceService.startMicrophoneMeter();
      return;
    }
    if (directCallRef.current && directCallStatus === "connected") {
      const nextStream = await captureDirectStream("audio", inputDeviceId || undefined, nextNoiseSuppression);
      await directCallRef.current.replaceMicrophone(nextStream);
      directLocalStream?.getTracks().forEach((track) => track.stop());
      setDirectLocalStream(nextStream);
    }
  };

  const changeVoiceInput = async (deviceId: string) => {
    setSelectedInput(deviceId);
    window.localStorage.setItem("concord.voice.input", deviceId);
    try { await applyVoicePreferences(deviceId, noiseSuppression); setNotice("Microfone atualizado."); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Não foi possível trocar o microfone."); }
  };

  const changeNoiseSuppression = async (enabled: boolean) => {
    setNoiseSuppression(enabled);
    window.localStorage.setItem("concord.voice.noiseSuppression", String(enabled));
    try { await applyVoicePreferences(selectedInput, enabled); setNotice(enabled ? "Supressão de ruído ativada." : "Supressão de ruído desativada."); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Não foi possível atualizar a supressão de ruído."); }
  };

  const changeVoiceOutput = (deviceId: string) => {
    setSelectedOutput(deviceId);
    window.localStorage.setItem("concord.voice.output", deviceId);
    setNotice("Saída de voz atualizada.");
  };

  const toggleMuteAllParticipants = () => {
    const nextMuted = !mutedAllParticipants;
    setPeerVolumes((current) => Object.fromEntries(members.filter((member) => member.uid !== currentUser.uid).map((member) => [member.uid, nextMuted ? 0 : 1]).concat(Object.entries(current).filter(([uid]) => members.some((member) => member.uid === uid)))));
    setMutedAllParticipants(nextMuted);
    setNotice(nextMuted ? "Todos os participantes foram silenciados para você." : "O áudio de todos os participantes foi restaurado.");
  };
  ...
  const shareVoiceInvite = async () => {
    const inviteRoomId = voiceRoomId ?? (channel?.kind === "voice" ? channel.id : null);
    if (!community || !inviteRoomId) { setNotice("Abra uma sala de call antes de convidar alguém."); return; }
    const room = rooms.find((item) => item.id === inviteRoomId);
    if (!room) return;
    const inviteUrl = buildVoiceInviteUrl(community.id, room.id);
    try {
      const nativeShare = (navigator as Navigator & { share?: (data: ShareData) => Promise<void> }).share;
      if (nativeShare) { await nativeShare.call(navigator, { title: `Call · ${room.name}`, text: `Entre na call ${room.name} do Concord.`, url: inviteUrl }); setNotice("Convite pronto para compartilhar."); }
      else { await navigator.clipboard.writeText(inviteUrl); setNotice("Link da call copiado para a área de transferência."); }
    } catch (error) {
      if (error instanceof DOMException && error.name === "AbortError") return;
      try { await navigator.clipboard.writeText(inviteUrl); setNotice("Link da call copiado para a área de transferência."); }
      catch { setNotice("Não foi possível compartilhar o convite. Copie o endereço da página."); }
    }
  };
  ...
  const leaveVoiceRoom = async () => {
    ...
    setVoiceStream(null); setVoiceRoomId(null); setVoiceSessionId(null); setMuted(false); setLocalSpeaking(false); setScreenSharing(false); setScreenPreviewStream(null); setScreenViewerIds(new Set()); setRemoteStreams({}); setRemoteScreenStreams({}); setPeerQualities({}); setScreenVolumes({}); setPeerVolumes({}); setSelectedPeerVolumeId(null); setMutedAllParticipants(false); setVoiceAudioBlocked(false); setVoiceAudioUnlockVersion(0); previousScreenBroadcasters.current = new Set();
  ...
  const toggleVoice = async (room: FirebaseVoiceRoom) => {
    ...
    try {
      if (isCurrentRoom) { await leaveVoiceRoom(); }
        ...
        const localStream = await voiceService.captureMicrophone(selectedInput || undefined, noiseSuppression);
        const activeInputId = localStream.getAudioTracks()[0]?.getSettings().deviceId;
        if (activeInputId) setSelectedInput(activeInputId);
        void voiceService.listAudioDevices().then(setAudioDevices);
  ...
  return <div className="app-shell firebase-original-shell">
    ...
    {mobileDrawer && <button type="button" className="mobile-drawer-backdrop" aria-label="Fechar menu" onClick={() => setMobileDrawer(null)} />}

    <aside className={mobileDrawer === "channels" ? "channel-sidebar mobile-open" : "channel-sidebar"}>
      <div className="community-header"><div><strong>{community?.name ?? "Seu espaço"}</strong><span>Comunidade criativa</span></div><div className="community-header-actions"><button type="button" className="mobile-close-panel" aria-label="Fechar navegação" onClick={() => setMobileDrawer(null)}><X size={17} /></button><button type="button" className="desktop-more" aria-label="Mais opções"><ChevronDown size={17} /></button></div></div>
      <div className="mobile-community-switcher" aria-label="Comunidades disponíveis">{communities.map((item) => <button type="button" key={item.id} className={community?.id === item.id ? "active" : ""} onClick={() => { setCommunity(item); setChannel(null); setDirectFriendId(null); setActivePanel("chat"); setMobileDrawer(null); }} title={item.name}>{initials(item.name)}</button>)}<button type="button" onClick={() => { setMobileDrawer(null); openCreationDialog("community"); }} aria-label="Criar comunidade"><Plus size={16} /></button></div>
      ...
      <div className="user-panel"><button className="user-identity" onClick={() => setActivePanel("profile")} aria-label="Abrir perfil"><ProfileAvatar profile={profile} fallbackName={currentUser.displayName ?? "Conta"} className="user-panel-avatar" /><span className="online-dot" /><div className="user-meta"><strong>{profile?.displayName ?? currentUser.displayName ?? "Conta Concord"}</strong><span>{profile?.publicId ?? "CON-00000000"}</span></div></button><button className={muted ? "control-active" : ""} onClick={toggleMute} aria-label="Alternar microfone"><Mic size={15} /></button><button className="logout-button" onClick={() => void auth.logout()} aria-label="Sair da conta" title="Sair da conta"><LogOut size={16} /></button></div>
    </aside>

    <main className="content-area">
      <header className="content-header"><div className="mobile-nav-actions"><button type="button" aria-label="Abrir canais" onClick={() => setMobileDrawer("channels")}><Compass size={17} /></button><button type="button" aria-label="Abrir amigos" onClick={() => { setActivePanel("friends"); setMobileDrawer(null); }}><Users size={17} /></button><button type="button" aria-label="Abrir perfil" onClick={() => { setActivePanel("profile"); setMobileDrawer(null); }}><Settings size={17} /></button></div><div className="channel-title"><div className="title-symbol"><Hash size={18} /></div><div><h2>{directFriendId ? "Mensagem direta" : channel?.name ?? "geral"}</h2><span>{channel?.kind === "voice" ? `Sala de voz em tempo real${isOnline ? "." : " · Offline — mensagens serão sincronizadas depois."}` : "Um espaço para começar qualquer conversa."}</span></div></div><div className="header-actions"><PwaInstallButton compact /><button title="Ativar notificações de mensagens" onClick={() => void enableVoiceNotifications()} aria-label="Ativar notificações de mensagens"><Bell size={17} />{Object.values(channelUnread).reduce((total, count) => total + count, 0) > 0 && <span className="header-unread-dot" />}</button><button title="Abrir Amigos" onClick={() => { setActivePanel("friends"); setSocialOpen(false); }}><Search size={17} /></button><button type="button" title="Abrir membros" aria-expanded={mobileDrawer === "members" || membersOpen} onClick={() => { if (window.matchMedia("(max-width: 760px)").matches) { setMembersOpen(true); setMobileDrawer("members"); } else { setMembersOpen((open) => !open); } }}><Users size={17} /></button><div className="header-divider" /><button type="button" className="profile-chip" onClick={() => { setActivePanel("profile"); setMobileDrawer(null); }}><span className="avatar profile-avatar-shell">{profile?.avatarUrl ? <img src={profile.avatarUrl} alt="" /> : <span>{initials(profile?.displayName ?? currentUser.displayName ?? "Conta")}</span>}<span className="online-dot" /></span><span>{profile?.displayName ?? currentUser.displayName ?? "Conta Concord"}</span><ChevronDown size={14} /></button></div></header>
      {notice && <div className="firebase-notice" role="status">{notice}</div>}
      {activePanel === "chat" ? <>
      {directFriendId ? <><section className="message-area"><div className="direct-chat-toolbar"><div className="direct-chat-identity"><ProfileAvatar profile={activeFriend} fallbackName="Amigo" /><div><strong>{activeFriend?.displayName ?? "Conversa direta"}</strong><small>{activeFriend?.presence === "online" ? "Online" : activeFriend?.presence === "away" ? "Ocupado" : "Indisponível"}</small></div></div><div className="direct-chat-actions"><div className="direct-call-actions" role="group" aria-label="Controles da chamada"><button className="direct-call-button" onClick={() => void startDirectCall("audio")} disabled={directCallStatus !== "idle" && directCallStatus !== "ended"} aria-label="Iniciar chamada de áudio" title="Chamada de áudio"><Headphones size={16} /><span>Áudio</span></button><button className={directScreenSharing ? "direct-call-button direct-call-screen-active" : "direct-call-button"} onClick={() => void toggleDirectScreen()} disabled={directCallStatus !== "connected"} aria-label={directScreenSharing ? "Parar compartilhamento de tela" : "Compartilhar tela na chamada"} title={directScreenSharing ? "Parar compartilhamento" : "Compartilhar tela"}><Video size={16} /><span>{directScreenSharing ? "Parar tela" : "Tela"}</span></button>{directCallStatus !== "idle" && directCallStatus !== "ended" ? <button className="direct-call-button direct-call-end" onClick={() => void endDirectCall()} aria-label="Encerrar chamada" title="Encerrar chamada"><PhoneOff size={16} /><span>Encerrar</span></button> : null}</div><div className="direct-utility-actions"><button onClick={() => setSocialOpen(true)} aria-label="Convidar para o grupo" title="Convidar"><UserPlus size={16} /></button><button onClick={() => { if (directFriendId) void deleteDirect(directFriendId); }} aria-label="Apagar conversa" title="Apagar conversa"><X size={16} /></button></div></div></div>{hasVideoTrack(directScreenStream) && <DraggableScreenPanel stream={directScreenStream} title="Sua tela" label="Você está compartilhando a tela" onClose={() => void closeDirectScreenForEveryone()} />}{hasVideoTrack(directRemoteScreenStream) && <DraggableScreenPanel stream={directRemoteScreenStream} title={activeFriend?.displayName ?? "Seu amigo"} label={`Tela de ${activeFriend?.displayName ?? "seu amigo"}`} volume={directScreenVolume} muted={false} outputDeviceId={selectedOutput || undefined} onVolumeChange={setDirectScreenVolume} onClose={() => void closeDirectScreenForEveryone()} />}<audio ref={directAudioRef} autoPlay playsInline aria-label="Áudio remoto da chamada" />{directRemoteStream && directAudioBlocked && <button className="direct-audio-unlock" onClick={() => void enableDirectAudio()}><Volume2 size={15} /> Ativar áudio</button>}<div className="channel-intro"><div className="intro-symbol"><MessageCircle size={27} /></div><h1>Conversa com {activeFriend?.displayName ?? "seu amigo"}</h1><p>Mensagens diretas com sua conexão.</p><div className="intro-rule" /></div><div className="message-list">{directMessages.length ? directMessages.map((item) => <article className="message-row" key={item.id}><span className="avatar bg-blue-200 text-blue-900">{initials(item.authorId)}</span><div className="message-copy"><div className="message-author"><strong>{item.authorId === currentUser.uid ? "Você" : item.authorId}</strong><span>@conexão</span><time>agora</time></div><p>{item.body}</p></div></article>) : <div className="empty-state">Nenhuma mensagem nesta conversa ainda.</div>}</div></section><div className="composer-wrap"><div className="composer"><Input value={directBody} onChange={(event) => setDirectBody(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void sendDirect(); }} placeholder="Mensagem direta" /><button className="send-button" onClick={() => void sendDirect()} aria-label="Enviar mensagem direta"><Send size={17} /></button></div></div></> : channel?.kind === "voice" ? <section className="message-area firebase-voice-original-stage" data-testid="voice-call-stage"><div className="channel-intro"><div className="intro-symbol"><Headphones size={27} /></div><h1>{channel.name}</h1><p>{members.length}/8 participante(s) sincronizado(s) pelo Firestore.</p><div className="intro-rule" /></div><div className="firebase-member-grid">{members.length ? members.map((member) => { const quality = peerQualities[member.uid]; return <div className={member.isSpeaking || (member.uid === currentUser.uid && localSpeaking) ? "firebase-member speaking call-member-speaking" : "firebase-member"} data-testid="voice-participant-card" data-self={member.uid === currentUser.uid ? "true" : "false"} key={member.uid} title={member.uid === currentUser.uid ? "Seu microfone" : `Clique para ajustar o volume de ${voiceProfiles[member.uid]?.displayName ?? member.displayName}`} role={member.uid === currentUser.uid ? undefined : "button"} tabIndex={member.uid === currentUser.uid ? undefined : 0} onClick={() => { if (member.uid !== currentUser.uid) setSelectedPeerVolumeId((current) => current === member.uid ? null : member.uid); }} onKeyDown={(event) => { if (member.uid !== currentUser.uid && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); setSelectedPeerVolumeId((current) => current === member.uid ? null : member.uid); } }}><span className="participant-avatar-wrap"><ProfileAvatar profile={{ ...member, ...voiceProfiles[member.uid] }} fallbackName={member.displayName} className={member.isSpeaking || (member.uid === currentUser.uid && localSpeaking) ? "speaking-avatar" : ""} /><span className={`participant-audio-indicator ${member.muted ? "is-muted" : member.isSpeaking || (member.uid === currentUser.uid && localSpeaking) ? "is-speaking" : "is-idle"}`} aria-label={member.muted ? "Microfone mutado" : member.isSpeaking || (member.uid === currentUser.uid && localSpeaking) ? "Falando agora" : "Microfone ativo"}>{member.muted ? <MicOff size={12} /> : <Mic size={12} />}</span></span><strong>{voiceProfiles[member.uid]?.displayName ?? member.displayName}</strong>{member.isSpeaking || (member.uid === currentUser.uid && localSpeaking) ? <span className="talking-label">Falando</span> : null}<span className={`voice-quality voice-quality-${quality?.level ?? "unknown"}`} title={`${qualityLabel(quality)}${quality?.ping !== null && quality?.ping !== undefined ? ` · ${quality.ping} ms` : ""}`}><Signal size={13} /><span>{quality?.ping !== null && quality?.ping !== undefined ? `${quality.ping} ms` : "--"}</span></span></div>; }) : <div className="empty-state">Ninguém na sala ainda.</div>}</div>{selectedPeerVolumeId && selectedPeerVolumeId !== currentUser.uid && members.some((member) => member.uid === selectedPeerVolumeId) && <div className="peer-volume-panel" role="group" aria-label="Volume individual do participante"><div className="peer-volume-heading"><Volume2 size={15} /><strong>Volume de {voiceProfiles[selectedPeerVolumeId]?.displayName ?? members.find((member) => member.uid === selectedPeerVolumeId)?.displayName ?? "participante"}</strong><output>{Math.round((peerVolumes[selectedPeerVolumeId] ?? 1) * 100)}%</output></div><input data-testid="peer-volume-slider" type="range" min="0" max="2" step="0.05" value={peerVolumes[selectedPeerVolumeId] ?? 1} onChange={(event) => setPeerVolumes((current) => ({ ...current, [selectedPeerVolumeId]: Number(event.target.value) }))} aria-label="Volume individual do participante" /></div>}<RemoteAudioMixer streams={remoteStreams} volumes={peerVolumes} unlockVersion={voiceAudioUnlockVersion} outputDeviceId={selectedOutput || undefined} onBlocked={() => setVoiceAudioBlocked(true)} />{voiceAudioBlocked && <button className="voice-audio-unlock" type="button" onClick={() => { setVoiceAudioBlocked(false); setVoiceAudioUnlockVersion((current) => current + 1); }}><Volume2 size={15} /> Ativar áudio dos participantes</button>}{screenPreviewStream && <DraggableScreenPanel stream={screenPreviewStream} title="Sua tela" label="Você está compartilhando a tela" onClose={() => void closeGroupScreen()} />}{members.filter((member) => member.screenSharing && member.uid !== currentUser.uid && screenViewerIds.has(member.uid) && hasVideoTrack(remoteScreenStreams[member.uid] ?? null)).map((member) => <DraggableScreenPanel key={member.uid} stream={remoteScreenStreams[member.uid]!} title={member.displayName} label={`Tela de ${member.displayName}`} volume={screenVolumes[member.uid] ?? 1} muted={false} outputDeviceId={selectedOutput || undefined} onVolumeChange={(value) => setScreenVolumes((current) => ({ ...current, [member.uid]: value }))} onClose={() => toggleScreenViewer(member.uid)} />)}{members.some((member) => member.screenSharing && member.uid !== currentUser.uid && remoteScreenStreams[member.uid] && !screenViewerIds.has(member.uid)) && <div className="screen-reopen-controls" aria-label="Controles de telas compartilhadas"><span><Video size={14} /> Telas disponíveis</span>{members.filter((member) => member.screenSharing && member.uid !== currentUser.uid && remoteScreenStreams[member.uid] && !screenViewerIds.has(member.uid)).map((member) => <button type="button" className="screen-reopen-button" key={`reopen-${member.uid}`} onClick={() => toggleScreenViewer(member.uid)}><Video size={14} /> Reabrir {member.displayName}</button>)}</div>}<div className="voice-chat-redirect"><MessageCircle size={16} /><span>O chat desta sala foi unificado com o canal Geral da comunidade.</span><button type="button" onClick={() => { const general = channels.find((item) => item.kind === "text"); if (general) setChannel(general); }}>Abrir Geral</button></div>{voiceRoomId ? <div className="call-actions firebase-voice-actions"><button className="call-invite-button" onClick={() => void shareVoiceInvite()} aria-label="Convidar por link"><Link2 size={16} /><span>Convidar</span></button><button data-testid="mute-all-button" className={mutedAllParticipants ? "control-active" : "call-invite-button"} onClick={toggleMuteAllParticipants} aria-label={mutedAllParticipants ? "Restaurar áudio de todos" : "Mutar todos os participantes"} title={mutedAllParticipants ? "Restaurar todos" : "Mutar todos"}><MicOff size={16} /><span>{mutedAllParticipants ? "Restaurar" : "Mutar todos"}</span></button><button className={muted ? "control-active" : ""} onClick={toggleMute} aria-label="Mutar microfone"><Mic size={16} /></button><button className={screenSharing ? "control-active" : ""} onClick={() => void toggleScreen()} aria-label="Compartilhar tela"><Video size={16} /></button><button className="disconnect" onClick={() => void leaveVoiceRoom()} aria-label="Sair da sala"><PhoneOff size={16} /></button></div> : <div className="call-actions firebase-voice-actions"><button className="join-call-button" onClick={() => activeVoiceRoom && void toggleVoice(activeVoiceRoom)} disabled={!activeVoiceRoom}><Headphones size={16} /> Entrar na chamada</button><button className="call-invite-button" onClick={() => void shareVoiceInvite()} aria-label="Convidar por link"><Link2 size={16} /><span>Convidar por link</span></button></div>}</section> : <><section className="message-area"><div className="channel-intro"><div className="intro-symbol"><Hash size={27} /></div><h1>Bem-vindo ao #{channel?.name ?? "geral"}</h1><p>Este é o começo do canal. Um bom lugar para dizer olá.</p><div className="intro-rule" /></div><div className="message-list">{messages.length ? messages.map((item) => <article className="message-row group-message-row" key={item.id}>{item.authorId === currentUser.uid && <button type="button" className="message-delete-button" onClick={() => void deleteMessage(item)} aria-label="Apagar mensagem" title="Apagar mensagem"><Trash2 size={14} /></button>}<ProfileAvatar profile={messageProfiles[item.authorId] ?? (item.authorId === currentUser.uid ? profile : null)} fallbackName={item.authorId === currentUser.uid ? currentUser.displayName ?? "Você" : item.authorName ?? "Participante"} /><div className="message-copy"><div className="message-author"><strong>{item.authorId === currentUser.uid ? "Você" : messageProfiles[item.authorId]?.displayName ?? item.authorName ?? item.authorId}</strong><span>@{messageProfiles[item.authorId]?.publicId ?? "concord"}</span><time>agora</time></div><p>{item.body}</p></div></article>) : <article className="message-row"><span className="avatar bg-blue-100 text-blue-900">CO</span><div className="message-copy"><div className="message-author"><strong>Concord</strong><span>@concord</span><time>agora</time></div><p>Bem-vindo ao Concord. Esta é a primeira mensagem deste canal.</p></div></article>}</div></section><div className="composer-wrap"><div className="composer"><button aria-label="Mais opções"><Plus size={19} /></button><Input value={body} onChange={(event) => setBody(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void sendMessage(); }} placeholder={"Mensagem em #" + (channel?.name ?? "geral")} /><button onClick={() => void sendMessage()} className="send-button" aria-label="Enviar mensagem"><Send size={17} /></button></div><span className="composer-hint">Enter para enviar <span>•</span> Shift + Enter para nova linha</span></div></>}
      </> : activePanel === "friends" ? <FriendsPanel currentUid={currentUser.uid} searchValue={search} results={results} friendships={friendships} friendProfiles={friendProfiles} invites={invites} onOpenChat={(uid) => { setDirectFriendId(uid); setActivePanel("chat"); }} onSearchValueChange={setSearch} onSearch={() => void searchProfiles()} onAddFriend={(target) => void addFriend(target)} onRespondFriend={(request, status) => void respondFriend(request, status)} onRespondInvite={(invite, status) => void respondInvite(invite, status)} /> : activePanel === "profile" ? <ProfilePanel profile={profile} email={currentUser.email ?? ""} onSave={(name, avatarUrl) => void saveDisplayName(name, avatarUrl)} onUpload={uploadAvatar} /> : <SettingsPanel audioDevices={audioDevices} selectedInput={selectedInput} selectedOutput={selectedOutput} noiseSuppression={noiseSuppression} micLevel={micLevel} onInputChange={changeVoiceInput} onOutputChange={changeVoiceOutput} onNoiseChange={changeNoiseSuppression} onRefreshDevices={() => { void voiceService.listAudioDevices().then(setAudioDevices); }} />}
    </main>

    <aside className={mobileDrawer === "members" ? "member-sidebar mobile-open" : membersOpen ? "member-sidebar" : "member-sidebar collapsed"}><div className="member-heading"><span>{voiceRoomId ? "NA CHAMADA" : "CONEXÕES"} — {voiceRoomId ? members.length : friendships.filter((item) => item.status === "accepted").length}</span><div className="member-heading-actions"><button type="button" className="mobile-close-panel" onClick={() => setMobileDrawer(null)} aria-label="Fechar membros"><X size={17} /></button><button type="button" onClick={() => setNotice("Membros sincronizados pelo Firebase.")} aria-label="Mais opções"><MoreHorizontal size={17} /></button></div></div><div className="member-group"><span className="member-role">{voiceRoomId ? "PARTICIPANTES DA SALA" : "CONEXÕES"}</span>{voiceRoomId ? (members.length ? members.map((member) => <div className="member-card call-member-card" key={member.uid}><ProfileAvatar profile={{ ...member, ...voiceProfiles[member.uid] }} fallbackName={member.displayName} /><div><strong>{voiceProfiles[member.uid]?.displayName ?? member.displayName}{member.uid === currentUser.uid ? " (Você)" : ""}</strong><span>{member.isSpeaking ? "Falando agora" : member.muted ? "Microfone mutado" : "Na chamada"}</span></div></div>) : <div className="empty-state">Ninguém entrou na chamada ainda.</div>) : <>{friendships.filter((item) => item.status === "accepted").map((item) => <button className="member-card" key={item.id} onClick={() => { setDirectFriendId(item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId); setActivePanel("chat"); setMobileDrawer(null); }}><ProfileAvatar profile={friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]} fallbackName="Conexão" /><div><strong>{friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]?.displayName ?? "Conexão"}</strong><span>{friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]?.presence === "online" ? "Online" : friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]?.presence === "away" ? "Ocupado" : "Indisponível"}</span></div></button>)}{!friendships.filter((item) => item.status === "accepted").length && <div className="empty-state">Nenhuma conexão aceita.</div>}</>}</div>{voiceRoomId && <div className="call-dock"><div className="call-status"><span className="call-pulse" /><div><strong>{rooms.find((room) => room.id === voiceRoomId)?.name ?? "Sala de voz"}</strong><span>{members.length || 1}/8 pessoa(s) na sala</span>{mutedAllParticipants && <span className="muted-all-status" data-testid="muted-all-status"><MicOff size={12} /> Mutar Todos ativo</span>}</div><button className="call-link-button" onClick={() => void shareVoiceInvite()} aria-label="Convidar por link" title="Convidar por link"><Link2 size={15} /><span>Convidar</span></button></div><div className="call-actions"><button className={muted ? "control-active" : ""} onClick={toggleMute} aria-label="Mutar microfone"><Mic size={16} /></button><button className={screenSharing ? "control-active" : ""} onClick={() => void toggleScreen()} aria-label="Compartilhar tela"><Video size={16} /></button><button className="disconnect" onClick={() => void leaveVoiceRoom()} aria-label="Sair da call"><X size={16} /></button></div></div>}</aside>
