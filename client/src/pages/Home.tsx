...
function Workspace({ onLogout, userName, userId, publicId }: { onLogout: () => void; userName: string; userId?: number; publicId?: string }) {
  ...
  const joinVoice = async (voiceId: number, voiceName: string) => { try { if (activeVoiceId === voiceId) { voiceStream?.getTracks().forEach((track) => track.stop()); leaveVoiceMutation.mutate({ channelId: voiceId }); webrtc.dispose(); setVoiceStream(null); setActiveVoiceId(null); setMuted(false); toast.success(`Você saiu de ${voiceName}`); return; } voiceStream?.getTracks().forEach((track) => track.stop()); const stream = await webrtc.captureMicrophone(); await joinVoiceMutation.mutateAsync({ channelId: voiceId }); setVoiceStream(stream); setActiveVoiceId(voiceId); setActiveChannel(voiceName); setView("home"); setMobilePanel("closed"); toast.success(`Você entrou em ${voiceName}`); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível entrar na sala de voz."); } };
    ...
    <div className="app-shell">
      ...
      <main className="content-area">
        {view === "friends" ? <FriendsView onOpenChannels={() => setMobilePanel("channels")} onOpenProfile={() => setProfileOpen(true)} /> : activeDm ? <DmView name={activeDm} threadId={dmThreadId} opening={openDm.isPending} openError={openDm.isError} onOpenChannels={() => setMobilePanel("channels")} onOpenFriends={() => setView("friends")} onOpenProfile={() => setProfileOpen(true)} /> : <>
          ...
          {activeVoiceId && <section className="voice-call-panel" aria-label="Controles da sala de voz"><div className="voice-call-heading"><div className="voice-call-icon"><Volume2 size={19} /></div><div><span className="voice-call-kicker">VOCÊ ESTÁ NA CALL</span><h3>{voiceItems.find((voice) => voice.id === activeVoiceId)?.name ?? "Sala de voz"}</h3><p>{voiceParticipantsQuery.data?.length ?? 1} participante(s) conectado(s)</p></div><span className="voice-call-live">AO VIVO</span></div><div className="voice-call-members">{voiceParticipantsQuery.data?.slice(0, 6).map((entry) => <span key={entry.member.id} title={entry.user.name ?? "Conta Concord"}>{(entry.user.name ?? "CO").slice(0, 2).toUpperCase()}</span>)}{!voiceParticipantsQuery.data?.length && <span className="voice-member-empty">Você</span>}</div><div className="voice-call-controls"><button className={muted ? "is-active" : ""} onClick={() => { voiceStream?.getAudioTracks().forEach((track) => { track.enabled = muted; }); setMuted(!muted); }}><Mic size={17} /><span>{muted ? "Ativar mic" : "Mutar"}</span></button><button className={screenSharing ? "is-active" : ""} onClick={() => void toggleScreenShare()}><Video size={17} /><span>{screenSharing ? "Parar tela" : "Compartilhar tela"}</span></button><button className="leave-call" onClick={leaveActiveVoice}><PhoneOff size={17} /><span>Sair da call</span></button></div></section>}
