  ...
  return <div className="app-shell firebase-original-shell">
    ...
    <aside className="channel-sidebar">
      ...
      <div className="channel-scroll">
        ...
        <div className="channel-group dm-group"><div className="group-label"><span>MENSAGENS DIRETAS</span><Plus size={13} /></div>{friendships.filter((item) => item.status === "accepted").length ? friendships.filter((item) => item.status === "accepted").map((item) => <button key={item.id} className="channel-link dm-link" onClick={() => setDirectFriendId(item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId)}><ProfileAvatar profile={friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]} fallbackName="Conexão" /><span>{friendProfiles[item.requesterId === currentUser.uid ? item.addresseeId : item.requesterId]?.displayName ?? "Conexão ativa"}</span></button>) : <div className="dm-status">Nenhuma amizade aceita ainda.</div>}</div>
