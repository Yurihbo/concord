    ...
    <div className="app-shell">
      ...
      <aside className={`channel-sidebar ${mobilePanel === "channels" ? "mobile-open" : ""}`}>
        ...
        <div className="channel-scroll">
          ...
          {channelsQuery.isLoading ? <div className="dm-status">Carregando canais...</div> : channelsQuery.isError ? <div className="dm-status dm-error">Não foi possível carregar canais.</div> : channelItems.length ? <div className="channel-group"><div className="group-label"><span>CANAIS</span><Plus size={13} /></div>{channelItems.map((channel) => <button key={channel.id} className={`channel-link ${activeChannel === channel.name && view === "home" ? "active" : ""}`} onClick={() => { setActiveChannel(channel.name); setActiveDm(null); setView("home"); setMobilePanel("closed"); }}><Hash size={15} />{channel.name}</button>)}</div> : channels.map((group) => <div className="channel-group" key={group.category}><div className="group-label"><span>{group.category}</span><Plus size={13} /></div>{group.items.map((channel) => <button key={channel.name} className={`channel-link ${activeChannel === channel.name && view === "home" ? "active" : ""}`} onClick={() => { setActiveChannel(channel.name); setView("home"); setMobilePanel("closed"); }}><Hash size={15} />{channel.name}{channel.name === "geral" && <span className="channel-live" />}</button>)}</div>)}
