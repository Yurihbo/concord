...
import { Bell, ChevronDown, Compass, Download, FileText, GripVertical, Hash, Headphones, Home as HomeIcon, LogOut, MessageCircle, Mic, MoreHorizontal, Paperclip, PhoneOff, Plus, Radio, Search, Send, Settings, Signal, Sparkles, UserPlus, Users, Video, Volume2, WandSparkles, X } from "lucide-react";
...
function VoiceChatPanel({ messages, members, currentUid, body, file, sending, fileInputRef, onBodyChange, onFileChange, onSend }: { messages: FirebaseMessage[]; members: FirebaseVoiceMember[]; currentUid: string; body: string; file: File | null; sending: boolean; fileInputRef: React.RefObject<HTMLInputElement | null>; onBodyChange: (value: string) => void; onFileChange: (file: File | null) => void; onSend: () => void }) {
  return <section className="voice-chat-panel" aria-label="Chat da chamada"><div className="voice-chat-heading"><div><span className="firebase-kicker">CHAT DA CHAMADA</span><strong>Converse enquanto fala</strong></div><MessageCircle size={18} /></div><div className="voice-chat-messages">{messages.length ? messages.map((message) => { const author = message.authorId === currentUid ? "Você" : message.authorName ?? members.find((member) => member.uid === message.authorId)?.displayName ?? "Participante"; return <article className="voice-chat-message" key={message.id}><span className="avatar bg-blue-200 text-blue-900">{initials(author)}</span><div><div className="voice-chat-author"><strong>{author}</strong><time>{message.createdAt ? new Date(String(message.createdAt)).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "agora"}</time></div><p>{message.body}</p>{message.fileUrl && <a className="voice-chat-file" href={message.fileUrl} target="_blank" rel="noreferrer" download={message.fileName}><FileText size={14} /><span>{message.fileName ?? "Arquivo"}</span><Download size={13} /></a>}</div></article>; }) : <div className="voice-chat-empty">Nenhuma mensagem ainda. Compartilhe uma ideia ou arquivo com a sala.</div>}</div>{file && <div className="voice-chat-attachment"><Paperclip size={13} /><span>{file.name}</span><button type="button" onClick={() => onFileChange(null)} aria-label="Remover arquivo selecionado"><X size={13} /></button></div>}<div className="voice-chat-composer"><input ref={fileInputRef} type="file" hidden onChange={(event) => onFileChange(event.target.files?.[0] ?? null)} /><button type="button" onClick={() => fileInputRef.current?.click()} aria-label="Anexar arquivo" title="Anexar arquivo" disabled={sending}><Paperclip size={16} /></button><Input value={body} onChange={(event) => onBodyChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onSend(); } }} placeholder="Mensagem para a chamada" disabled={sending} /><button type="button" className="send-button" onClick={onSend} disabled={sending || (!body.trim() && !file)} aria-label="Enviar mensagem"><Send size={16} /></button></div></section>;
}
...
export default function FirebaseWorkspace() {
  ...
  const [voiceMessages, setVoiceMessages] = useState<FirebaseMessage[]>([]);
  const [voiceBody, setVoiceBody] = useState("");
  const [voiceFile, setVoiceFile] = useState<File | null>(null);
  const [voiceSending, setVoiceSending] = useState(false);
  const [voiceUnread, setVoiceUnread] = useState<Record<string, number>>({});
  const voiceSeenMessageIds = useRef<Map<string, Set<string>>>(new Map());
  ...
  useEffect(() => {
    if (!community || !channel || channel.kind !== "voice") { setVoiceMessages([]); return; }
    return subscribeToVoiceMembers(community.id, channel.id, setMembers, (error) => setNotice(error.message));
  }, [community, channel]);

  useEffect(() => {
    const roomId = channel?.kind === "voice" ? channel.id : null;
    if (!community || !roomId) { setVoiceMessages([]); return; }
    return subscribeToChannelMessages(community.id, roomId, (nextMessages) => {
      const existingSeen = voiceSeenMessageIds.current.get(roomId);
      if (!existingSeen) {
        voiceSeenMessageIds.current.set(roomId, new Set(nextMessages.map((message) => message.id)));
        setVoiceMessages(nextMessages);
        return;
      }
      ...
      const roomIsVisible = activePanel === "chat" && channel?.id === roomId;
      if (roomIsVisible) {
        setVoiceUnread((current) => current[roomId] ? { ...current, [roomId]: 0 } : current);
        return;
      }
      setVoiceUnread((current) => ({ ...current, [roomId]: (current[roomId] ?? 0) + incoming.length }));
      const firstAuthor = incoming[0]?.authorName ?? "Novo participante";
      setNotice(`${firstAuthor} enviou ${incoming.length === 1 ? "uma nova mensagem" : `${incoming.length} novas mensagens`} em ${channel?.name ?? "sua sala"}.`);
      if (typeof Notification !== "undefined" && Notification.permission === "granted" && document.visibilityState === "hidden") {
        new Notification(`Nova mensagem em ${channel?.name ?? "sua sala"}`, { body: incoming.length === 1 ? `${firstAuthor}: ${incoming[0]?.body ?? "Novo conteúdo"}` : `${incoming.length} novas mensagens recebidas.` });
      }
    }, (error) => setNotice(error.message));
  }, [activePanel, channel, community, currentUserId]);

  useEffect(() => {
    if (activePanel !== "chat" || channel?.kind !== "voice") return;
    setVoiceUnread((current) => current[channel.id] ? { ...current, [channel.id]: 0 } : current);
  }, [activePanel, channel]);
  ...
  const sendVoiceChat = async () => {
    const roomId = channel?.kind === "voice" ? channel.id : voiceRoomId;
    if (!community || !roomId || voiceSending || (!voiceBody.trim() && !voiceFile)) return;
    if (voiceFile && voiceFile.size > 25 * 1024 * 1024) { setNotice("O arquivo excede o limite de 25 MB."); return; }
    setVoiceSending(true);
    try {
      const attachment = voiceFile ? await uploadVoiceChatFile(community.id, roomId, currentUser.uid, voiceFile) : undefined;
      await sendVoiceChatMessage(community.id, roomId, currentUser.uid, voiceBody.trim() || `Arquivo enviado: ${voiceFile?.name ?? "anexo"}`, attachment, profile?.displayName ?? currentUser.displayName ?? "Conta Concord");
      setVoiceBody("");
      setVoiceFile(null);
      if (voiceFileInputRef.current) voiceFileInputRef.current.value = "";
    } catch (error) { setNotice(error instanceof Error ? error.message : "Não foi possível enviar a mensagem da sala."); }
