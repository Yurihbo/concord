...
function ScreenPreview({ stream, label = "Sua tela está sendo compartilhada", muted = true }: { stream: MediaStream; label?: string; muted?: boolean }) {
...
function DraggableScreenPanel({ stream, title, label, volume, onVolumeChange, onClose }: { stream: MediaStream; title: string; label: string; volume?: number; onVolumeChange?: (value: number) => void; onClose?: () => void }) {
...
export default function FirebaseWorkspace() {
  ...
  const startDirectCall = async (media: "audio" | "screen") => {
    ...
    try {
      ...
      const service = new FirebaseDirectCall({ callId, userId: currentUser.uid, media, localStream: local, onRemoteStream: (stream) => { setDirectRemoteStream(new MediaStream(stream.getTracks())); setDirectCallStatus("connected"); }, onScreenShareEnded: () => { setDirectScreenStream(null); setDirectScreenSharing(false); }, onError: (error) => setNotice(error.message) });
      directCallRef.current = service; setPendingDirectSignals([]); setDirectLocalStream(local); setDirectScreenStream(media === "screen" ? local : null); setDirectScreenSharing(media === "screen"); setDirectRemoteStream(null); setDirectAudioBlocked(false); setDirectCallId(callId); setDirectCallDirection("outgoing"); setDirectCallStatus("ringing");
  ...
  const endDirectCall = async () => {
    ...
    try { if (directCallId) await updateDirectCall(directCallId, "ended"); }
      ...
      directScreenStream?.getTracks().forEach((track) => track.stop());
      directRemoteStream?.getTracks().forEach((track) => track.stop());
      setDirectLocalStream(null); setDirectScreenStream(null); setDirectScreenSharing(false); setDirectRemoteStream(null); setDirectVolume(1); setDirectAudioBlocked(false); setDirectCallId(null); setDirectCallDirection(null); setDirectCallMedia("audio"); setDirectCallStatus("ended"); setPendingDirectSignals([]); handledDirectSignalIds.current.clear();
    }
  };
  ...
  const toggleVoice = async (room: FirebaseVoiceRoom) => {
    ...
    try {
      if (isCurrentRoom) { await leaveVoiceRoom(); }
        ...
        meshRef.current = new FirebaseVoiceMesh({ roomId: room.id, userId: currentUser.uid, localStream, onRemoteStream: (peerId, stream) => setRemoteStreams((current) => ({ ...current, [peerId]: stream })), onError: (error) => setNotice(error.message), onScreenShareEnded: () => { setScreenSharing(false); setScreenPreviewStream(null); if (!leavingVoiceRef.current) void updateOwnScreenState(false); } });
