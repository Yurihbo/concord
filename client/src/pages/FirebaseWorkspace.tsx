...
function VoiceScreenPanel({ currentUid, members, screenStreams, viewers, screenVolumes, onToggleViewer, onVolumeChange }: { currentUid: string; members: FirebaseVoiceMember[]; screenStreams: Record<string, MediaStream>; viewers: Set<string>; screenVolumes: Record<string, number>; onToggleViewer: (peerId: string) => void; onVolumeChange: (peerId: string, value: number) => void }) {
  const broadcasters = members.filter((member) => member.screenSharing && member.uid !== currentUid);
  if (!broadcasters.length) return null;
  return <section className="voice-screen-panel" aria-label="Transmissões de tela"><div className="voice-screen-panel-heading"><div><span className="firebase-kicker">TRANSMISSÃO AO VIVO</span><strong>{broadcasters.length === 1 ? "Uma pessoa está compartilhando a tela" : `${broadcasters.length} pessoas estão compartilhando a tela`}</strong></div><Video size={18} /></div>{broadcasters.map((member) => { const viewing = viewers.has(member.uid); const hasStream = Boolean(screenStreams[member.uid]); const volume = screenVolumes[member.uid] ?? 1; return <article className="voice-screen-card" key={member.uid}><div className="voice-screen-card-heading"><div><strong>{member.displayName}</strong><small>{hasStream ? "Tela disponível no painel flutuante" : "Conectando à tela..."}</small></div><Button variant="outline" onClick={() => onToggleViewer(member.uid)}>{viewing ? "Ocultar tela" : "Exibir tela"}</Button></div>{hasStream && <label className="voice-screen-volume"><Volume2 size={14} /><span>Volume da tela</span><input type="range" min="0" max="1" step="0.05" value={volume} onChange={(event) => onVolumeChange(member.uid, Number(event.target.value))} aria-label={`Volume da tela de ${member.displayName}`} /><output>{Math.round(volume * 100)}%</output></label>}</article>; })}</section>;
}
